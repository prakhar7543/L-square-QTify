...
var _SliderThumbDataAttributes = require("../thumb/SliderThumbDataAttributes");
...
function focusThumb(thumbIndex, sliderRef, setActive) {
  ...
  if (activeEl == null || !sliderRef.current.contains(activeEl) || Number(activeEl.getAttribute(_SliderThumbDataAttributes.SliderThumbDataAttributes.index)) !== thumbIndex) {
    // TODO: possibly simplify with thumbRefs as it already exists
    sliderRef.current.querySelector(`[type="range"][${_SliderThumbDataAttributes.SliderThumbDataAttributes.index}="${thumbIndex}"]`).focus();
  }
  if (setActive) {
...
function useSliderRoot(parameters) {
  ...
  const controlStylesRef = React.useRef(null);
  ...
  thumbOffset = 0) => {
    ...
    const newPercentageValue = (0, _clamp.clamp)(valueRescaled * 100, percentageValues[closestThumbIndex - 1] + minPercentageDifference || -Infinity, percentageValues[closestThumbIndex + 1] - minPercentageDifference || Infinity);
    return {
      value: (0, _replaceArrayItemAtIndex.replaceArrayItemAtIndex)(values, closestThumbIndex, newValue),
      valueRescaled,
      percentageValues: (0, _replaceArrayItemAtIndex.replaceArrayItemAtIndex)(percentageValues, closestThumbIndex, newPercentageValue),
