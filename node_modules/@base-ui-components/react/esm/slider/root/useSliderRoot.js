...
import { SliderThumbDataAttributes } from '../thumb/SliderThumbDataAttributes.js';
...
export function focusThumb(thumbIndex, sliderRef, setActive) {
  ...
  if (activeEl == null || !sliderRef.current.contains(activeEl) || Number(activeEl.getAttribute(SliderThumbDataAttributes.index)) !== thumbIndex) {
    // TODO: possibly simplify with thumbRefs as it already exists
    sliderRef.current.querySelector(`[type="range"][${SliderThumbDataAttributes.index}="${thumbIndex}"]`).focus();
  }
  if (setActive) {
...
export function useSliderRoot(parameters) {
  ...
  thumbOffset = 0) => {
    ...
    const newPercentageValue = clamp(valueRescaled * 100, percentageValues[closestThumbIndex - 1] + minPercentageDifference || -Infinity, percentageValues[closestThumbIndex + 1] - minPercentageDifference || Infinity);
    return {
      value: replaceArrayItemAtIndex(values, closestThumbIndex, newValue),
      valueRescaled,
      percentageValues: replaceArrayItemAtIndex(percentageValues, closestThumbIndex, newPercentageValue),
